...
let dynamic = {
    controller: () => {
        ...
        return {
            ...
            route: m.prop(m.route())
        };
    },
    view: (ctrl) => {
        let icon;
        if (m.route() !== ctrl.route()) {
            ctrl.refreshIcon();
            ctrl.route(m.route());
        }
        icon = ctrl.icon();
