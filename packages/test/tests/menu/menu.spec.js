...
describe("Menu component", () => {
  ...
  it("option permanent", () => {
    const cmp = m(component, {permanent: true});
    const html = tidy(cmp, {
      ...defaultHtmlTidyOptions,
      wrap: false,
      indent: false
  ...
  it("child", () => {
    const cmp = m(component, {permanent: true}, m("span", "Child"));
    const html = tidy(cmp);
    expect(html).toContain("<span>Child</span>");
  });
  it("option z:0", () => {
    const cmp = m(component, {permanent: true, z: 0});
    const html = tidy(cmp, {
      ...defaultHtmlTidyOptions,
      wrap: false,
      indent: false
  ...
  it("option id", () => {
    const cmp = m(component, {permanent: true, id: "id-x"});
    const html = tidy(cmp);
    expect(html).toContain("id=\"id-x\"");
  });
  it("option class", () => {
    const cmp = m(component, {permanent: true, class: "class-x"});
    const html = tidy(cmp);
    expect(html).toContain("class-x\">");
  });
  it("option element", () => {
    const cmp = m(component, {permanent: true, element: "span"});
    const html = tidy(cmp);
    expect(html).toContain("<span ");
    expect(html).toContain("</span>");
  });
  it("option before", () => {
    const cmp = m(component, {permanent: true, before: m("span", "Before")});
    const html = tidy(cmp);
    expect(html).toContain("<span>Before</span>");
  });
  it("option after", () => {
    const cmp = m(component, {permanent: true, after: m("span", "After")});
    const html = tidy(cmp);
