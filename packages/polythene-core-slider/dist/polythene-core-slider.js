(function (global, factory) {
  ...
  const startDrag = (state, attrs, e) => {
    ...
    const endDrag = () => {
      ...
      if (polytheneCore.isClient) {
        polytheneCore.pointerMoveEvent.forEach(evt => window.removeEventListener(evt, drag));
        polytheneCore.pointerEndMoveEvent.forEach(evt => window.removeEventListener(evt, endDrag));
      }
    ...
    if (polytheneCore.isClient) {
      polytheneCore.pointerMoveEvent.forEach(evt => window.addEventListener(evt, drag));
      polytheneCore.pointerEndMoveEvent.forEach(evt => window.addEventListener(evt, endDrag));
    }
  ...
  const createSlider = (vnode, {
    ...
    }, interactiveTrack && !attrs.disabled && polytheneCore.pointerStartMoveEvent.reduce((acc, evt) => (acc[k[`on${evt}`]] = onStartTrack, acc), {})), [h("div", {
    ...
    }, !attrs.disabled && polytheneCore.pointerStartMoveEvent.reduce((acc, evt) => (acc[k[`on${evt}`]] = onInitDrag, acc), {}), attrs.events ? attrs.events : null, hasTicks ? {
      step: stepCount
