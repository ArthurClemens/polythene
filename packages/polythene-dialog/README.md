...
The dialog can be closed programmatically with `dialog.hide()` or `dialog.hide(dialogId)`.
...
~~~javascript
dialog.show(myFullscreenDialogOptions); // using default id
...
| **content**   | optional       | Mithril element | | Alternative content; replaces `body` or `vnode.children` |
