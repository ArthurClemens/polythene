...
var handleSubscriptions = function handleSubscriptions(vnode, which) {
  ...
  if (which === "mount") {
    ...
    attrs.closeOnEscape && polytheneCore.subscribe("keydown", state.handleEscape);
    setTimeout(function () {
      state.activateDismissTap();
    }, 0);
  } else {
    // unsubscribe("resize", state.update);
    attrs.closeOnEscape && polytheneCore.unsubscribe("keydown", state.handleEscape);
...
var showDrawer = function showDrawer(state, attrs) {
  ...
  if (attrs.backdrop) {
    ...
    polytheneCore.show(_extends({}, attrs, backdropTransitions.show({ el: state.backdropDom(), showDuration: attrs.showDuration || ANIMATION_DURATION, showDelay: attrs.showDelay })));
  }
  // Show pane
  return polytheneCore.show(_extends({}, attrs, transitions.show({ el: state.dom(), showDuration: attrs.showDuration || ANIMATION_DURATION, showDelay: attrs.showDelay }))).then(function () {
...
var hideDrawer = function hideDrawer(state, attrs) {
  ...
  if (attrs.backdrop) {
    var backdropTransitions = attrs.backdropTransitions || defaultBackdropTransitions;
    polytheneCore.hide(_extends({}, attrs, backdropTransitions.hide({ el: state.backdropDom(), hideDuration: attrs.hideDuration || ANIMATION_DURATION, hideDelay: attrs.hideDelay })));
  }
  // Hide pane
  return polytheneCore.hide(_extends({}, attrs, transitions.hide({ el: state.dom(), hideDuration: attrs.hideDuration || ANIMATION_DURATION, hideDelay: attrs.hideDelay }))).then(function () {
    state.visible(false);
    state.transitioning(false);
    if (attrs.onChange) {
      attrs.onChange({ visible: state.visible(), transitioning: state.transitioning() });
