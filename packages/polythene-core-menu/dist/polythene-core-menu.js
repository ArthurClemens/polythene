...
var positionMenu = function positionMenu(state, attrs) {
  if (!attrs.target || polytheneCore.isServer) {
    return;
  }
...
var showBackdrop = function showBackdrop(state, attrs) {
  if (!attrs.backdrop || polytheneCore.isServer) {
    return;
  }
  ...
  state.backdropEl = el;
  polytheneCore.show(_extends({}, attrs, backdropTransitions.show({ el: el, showDuration: attrs.showDuration || ANIMATION_DURATION, showDelay: attrs.showDelay })));
};
...
var hideBackdrop = function hideBackdrop(state, attrs) {
  ...
  polytheneCore.hide(_extends({}, attrs, backdropTransitions.hide({ el: el, hideDuration: attrs.hideDuration || ANIMATION_DURATION, hideDelay: attrs.hideDelay }))).then(function () {
    if (el.parentNode) {
...
var showMenu = function showMenu(state, attrs) {
  ...
  var el = state.dom();
  return polytheneCore.show(_extends({}, attrs, transitions.show({ el: el, showDuration: attrs.showDuration || ANIMATION_DURATION, showDelay: attrs.showDelay }))).then(function () {
    if (attrs.onChange) {
...
var hideMenu = function hideMenu(state, attrs) {
  ...
  var transitions = attrs.transitions || defaultTransitions;
  var el = state.dom();
  return polytheneCore.hide(_extends({}, attrs, transitions.hide({ el: el, hideDuration: attrs.hideDuration || ANIMATION_DURATION, hideDelay: attrs.hideDelay }))).then(function () {
    if (attrs.onChange) {
