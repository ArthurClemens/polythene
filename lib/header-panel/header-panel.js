define([
    ...
    createHeaderComponent = function (opts) {
        ...
        if (opts.toolbar) {
            opts.toolbar.className = [(opts.toolbar.className || null), tall ? tallClass : null].join(' ');
            return m.component(toolbar, opts.toolbar);
        } else if (opts.content) {
            return m(['.header', opts.animated ? ' .animate' : ''].join(' '), {
                    className: [(opts.className || null), tall ? tallClass : null].join(' ')
                }, opts.content);
        } else {
            return m('div', opts);
        }
    ...
    createView = function (ctrl, opts) {        
        ...
        header = opts.header ? createHeaderComponent(p.assign({}, opts.header, ctrl.headerConfig)) : null;
        tag = opts.tag || 'div';

        if (opts.config) {
            opts.configs.push(opts.config);
        ...
        if (opts.animated) pprops.animated = true;
        if (ctrl.fixed()) pprops.fixed = true;
        ...
        content = m('.outerContainer[vertical][layout]', {
            ...
            onscroll: scrollConfig.outer
                ...
                m('.headerContainer', {
                    ...
                    header,
                    (mode === 'seamed' || ctrl.shadow() === false) ? null :
                        m('.dropShadow')
                ]),
        ...
        controller: function (opts) {
            ...
            return {
                ...
                transformHeader: function (y) {
                    ...
                    p.observable.fire('header-transform', {
                        y: y,
                        height: headerHeight, 
